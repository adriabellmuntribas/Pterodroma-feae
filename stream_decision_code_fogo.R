...
# Ruta i fitxers
ruta <- "/Users/adriabellmuntribas/Desktop/TFM/QGIS/Gis project/26Nreprojected/fogo/produced_layers/stream_NDVI/taules_NDVI"
arxius <- c(
  "4_5_fogo_26N_cor_est.csv",
  "5_6_fogo_26N_cor_est.csv",
  "6_7_fogo_26N_cor_est.csv",
  "7_8_fogo_26N_cor_est.csv",
  "8_9_fogo_26N_cor_est.csv",
  "9_10_fogo_26N_cor_est.csv"
)
...
dades <- lapply(arxius, function(fitxer) {
  read_csv(file.path(ruta, fitxer)) %>%
    ...
    mutate(threshold_range = gsub("_fogo_26N_cor_est.csv", "", fitxer))
}) %>% bind_rows()
...
ggplot(ndvi_mitjanes, aes(x = threshold_numeric, y = mean_ndvi)) +
  ...
  geom_point(size = 2) +
  scale_x_continuous(breaks = 4:10, limits = c(4, 10)) +     # Thresholds de 4 a 10
  labs(
    x = "Threshold",
    y = "NDVI mitjà"
  ) +
